...
export function InvoiceBatchList({ batches }: { batches: Batch[] }) {
  ...
  if (batches.length === 0) {
    // 2026-08-08: "TO INVOICE KYU NAHI BANA PA RAHE" — the old copy here
    // ("All dispatched/delivered orders have already been invoiced.") was
    // misleading when the real reason is that NO order has been marked
    // Dispatched/Delivered yet (brand-new orders start life as Pending —
    // see Orders hub). Spell out what to actually do instead of implying
    // everything is already done.
    return (
      <p className="text-sm text-slate-400">
        No orders are ready to invoice right now. An order only shows up here once its status is changed to{" "}
        <strong>Dispatched</strong> or <strong>Delivered</strong> — go to the Orders hub, open Edit on the order, and
        update its Status, then come back here.
      </p>
    );
  }
